...
```php
\PimcoreVendureBridgeBundle\PimcoreVendureBridgeBundle::class => ['all' => true]
```
...
```yaml
imports:
- { resource: '@PimcoreVendureBridgeBundle/Resources/config/config.yaml'}
```
- in routes.yaml necessary to register routes from bundle
```yaml
PimcoreVendureBridgeBundle:
    resource: '@PimcoreVendureBridgeBundle/Resources/config/routes.yaml'
```

## Messenger
Messenger add data object to queue on `preUpdate / postDelete` event. Necessary is to implement on data object `\PimcoreVendureBridgeBundle\Models\PimcoreVendureInterface` interface
