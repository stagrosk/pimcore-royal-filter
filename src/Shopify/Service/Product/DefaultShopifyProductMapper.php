...
use App\Shopify\Model\Product\ShopifyProductVariant;
...
class DefaultShopifyProductMapper implements IShopifyProductMapper
{
    ...
    private function getMedias(Product $product): array
    {
        ...
        foreach ($images->getItems() as $image) {
            $medias[] = new ShopifyImageInput(Tool::getHostUrl() . $image->getImage()->getFrontendFullPath(), $image->getImage()->getFilename());
        }
